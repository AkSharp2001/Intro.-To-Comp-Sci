...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
    	...
    	int numDays = numberOfDaysInMonth[month - 2];
    	int forecast[] = new int[numDays];
    	int row = 0;
    	while (drywet[row][0] != longitute || drywet[row][1] != latitude) {
    		row++;
    	}
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
    	...
    	if (mode == WET) {
    		for (int i = 0; i < forecast.length - 1; i++) {
    			if (forecast[i] == WET && forecast[i + 1] == WET) {
    				spell++;
    			}
    			else {
    				spell = 1;
    			}
    			if (spell > longestSpell) {
    				longestSpell = spell;
    			}
    		}
    	}
    		...
    		for (int i = 0; i < forecast.length - 1; i++) {
    			if (forecast[i] == DRY && forecast[i + 1] == DRY) {
    				spell++;
    			}
    			else {
    				spell = 1;
